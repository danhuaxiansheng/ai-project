...
class WorldGenerator:
    ...
    def _generate_description(self, prompt: str, random_generator: random.Random) -> str:
        """生成世界描述"""
        if prompt:
    ...
    def _generate_civilization(self, random_generator: random.Random) -> Dict:
        ...
        return {
            ...
            "culture": {
                ...
                "values_name": cultural_values[random_generator.choice(list(cultural_values.keys()))],
    ...
    def _generate_civilization_description(
        ...
        return template.format(
            tech=tech_names[tech_level],
            social=social_names[social_structure]
        )
    ...
    def _generate_history(self, random_generator: random.Random, complexity: int) -> Dict:
        """生成历史事件"""
        # 定义历史时期
        eras = {
            "ancient": "远古时期",
        ...
        for era in selected_eras:
            ...
            for _ in range(num_events):
                ...
                event_template = random_generator.choice(event_types[event_type]["templates"])
                
                event_year = random_generator.randint(current_year, current_year + era_length)
                event_data = {
                    "year": event_year,
                    "type": event_type,
                    "type_name": event_types[event_type]["name"],
                    "description": self._generate_event_description(event_template, random_generator),
        ...
        # 生成时间线摘要
        history_data["timeline"] = self._generate_timeline_summary(history_data["events"])
    ...
    def _generate_event_description(self, template: str, random_generator: random.Random) -> str:
        """生成具体事件描述"""
        # 事件细节词库
        details = {
        ...
        for detail_type in detail_types:
            if detail_type in details:
                filled_details[detail_type] = random_generator.choice(details[detail_type])
    ...
    def _generate_races(self, random_generator: random.Random, complexity: int) -> Dict:
        ...
        for i in range(num_races):
            race_data = {
                ...
                "description": self._generate_race_description(race_traits, random_generator),
        ...
        for creature_type, type_info in creature_types.items():
            ...
            for _ in range(num_species):
                species_data = {
                    "name": self._generate_creature_name(creature_type, random_generator),
                    ...
                    "significance": self._generate_creature_significance(random_generator)
                }
        ...
        races_data["ecosystem"] = self._generate_ecosystem_description(
            races_data["creatures"],
            random_generator
        )
        
        return races_data

    def _generate_race_description(self, traits: Dict, random_generator: random.Random) -> str:
        """生成种族描述"""
        templates = [
        ...
        selected_traits = {
            t: random_generator.choice(list(traits[t].values()))[0]
            for t in trait_types
        }
        
        return random_generator.choice(templates).format(**selected_traits)

    def _generate_creature_name(self, creature_type: str, random_generator: random.Random) -> str:
        """生成生物名称"""
        prefixes = {
            "terrestrial": ["地", "山", "林", "沙"],
            "aerial": ["风", "云", "翼", "空"],
    ...
    def _generate_systems(self, random_generator: random.Random, complexity: int) -> Dict:
        ...
        if random_generator.random() < 0.7 or "magic" in focus_areas:
            magic_system = {
                "types": random_generator.sample(list(magic_types.items()), k=min(4, complexity)),
                "sources": random_generator.sample(list(magic_sources.items()), k=min(3, complexity)),
                "principles": random_generator.sample(magic_principles, k=min(3, complexity)),
                "limitations": random_generator.sample(magic_limitations, k=min(3, complexity)),
        ...
        if random_generator.random() < 0.7 or "technology" in focus_areas:
            tech_system = {
                "categories": {},
                "breakthroughs": self._generate_tech_breakthroughs(random_generator, complexity),
            ...
            for category, info in tech_categories.items():
                if random_generator.random() < 0.8:
                    tech_system["categories"][category] = {
                        "name": info["name"],
                        "types": random_generator.sample(info["types"], k=min(3, complexity)),
                        "development_level": random_generator.randint(1, 10),
                        "description": self._generate_tech_category_description(category, info["name"])
                    }
    ...
    def _generate_magic_schools(self, random_generator: random.Random, complexity: int) -> List[Dict]:
        """生成魔法流派"""
        school_templates = [
    ...
    def _generate_magic_techniques(self, focus: str, random_generator: random.Random, count: int) -> List[str]:
        """生成魔法技术"""
        technique_templates = {
        ...
        return random_generator.sample(base_techniques, k=min(len(base_techniques), count))

    def _generate_tech_breakthroughs(self, random_generator: random.Random, complexity: int) -> List[Dict]:
        """生成科技突破"""
